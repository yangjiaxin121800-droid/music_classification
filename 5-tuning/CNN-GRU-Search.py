...
def run_grid_search(X_train, X_val, y_train, y_val, device='cuda'):
    ...
    print(f"Search Space: {len(param_grid['cnn_dropout']) * len(param_grid['classifier_dropout']) * len(param_grid['weight_decay'])} combinations")
    ...
    for i, (beta2, cnn_drop, cls_drop, wd) in enumerate(param_combinations):
        print(f"\n{'='*60}")
        print(f"{i+1}/{len(param_combinations)}")
        print(f"Config: beta2={beta2}, cnn_dropout={cnn_drop}, cls_dropout={cls_drop}, weight_decay={wd}")
        print(f"{'='*60}")
